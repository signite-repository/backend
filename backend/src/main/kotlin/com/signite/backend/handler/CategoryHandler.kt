...
class CategoryHandler(
    ...
    fun getAll(req: ServerRequest): Mono<ServerResponse> {
        return Mono.just(req)
            .flatMap { categoryService.getCategoryAll().collectList() }
            .flatMap { ok().body(Mono.just(it)) }
    }

    fun getAllAndCache(req: ServerRequest) =
        Mono.just(req)
            .flatMap { categoryService.getAllAndCache() }
            .flatMap {
                ok().body(
                    Mono.just(it),
                )
    ...
    fun getAllPostByCategoryId(req: ServerRequest) =
        Mono.just(req)
            .flatMap {
                ...
                categoryService.getAllPostByCategoryId(
                    req.pathVariable("categoryId").toInt(),
                    offset,
                    limit,
                )
            }
