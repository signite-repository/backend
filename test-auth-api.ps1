...
Write-Host "=== Auth Service API Test ===" -ForegroundColor Green

# 1. Health Check
Write-Host "`n1. Testing Health Check..." -ForegroundColor Yellow
try {
    $health = Invoke-WebRequest -Uri http://localhost:8082/actuator/health -Method GET -UseBasicParsing
...
try {
    $register = Invoke-WebRequest -Uri http://localhost:8082/api/auth/register `
        -Method POST `
...
try {
    $login = Invoke-WebRequest -Uri http://localhost:8082/api/auth/login `
        -Method POST `
        -ContentType "application/json" `
        -Body $loginBody `
        -UseBasicParsing
    ...
    try {
        ...
        $profile = Invoke-WebRequest -Uri http://localhost:8082/api/auth/profile `
            -Method GET `
            -Headers $headers `
            -UseBasicParsing
